...
const modifierStage = async (requete, reponse, next) => {
  ...
  try {
    ...
    stage.titre =  requete.params.titre;
    stage.description = requete.params.description;
    stage.debut = requete.params.debut;
    stage.fin = requete.params.fin;
    stage.payant = requete.params.payant;
    stage.modalite = requete.params.modalite;
    stage.nom_entreprise = requete.params.nom_entreprise;
    stage.status = requete.params.status;
    await stage.save();
  } catch (err) {
    const erreur = new HttpErreur(err, 500);
    return next(erreur);
